...
#pragma omp target map(present, tofrom : arr[SMALL])
  ...
  // CHECK: omp{{v?}}target message: explicit extension not allowed: host address specified is 0x{{0*}}[[#LARGE_ADDR]] ([[#LARGE_BYTES]] bytes), but device allocation maps to host at 0x{{0*}}[[#SMALL_ADDR]] ([[#SMALL_BYTES]] bytes)
  // CHECK: omp{{v?}}target message: device mapping required by 'present' map type modifier does not exist for host address 0x{{0*}}[[#LARGE_ADDR]] ([[#LARGE_BYTES]] bytes)
  // CHECK: omp{{v?}}target error: Call to getTargetPointer returned null pointer ('present' map type modifier).
  // CHECK: omp{{v?}}target error: Call to targetDataBegin failed, abort target.
  // CHECK: omp{{v?}}target error: Failed to process data before launching the kernel.
