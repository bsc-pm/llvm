...
#if defined(_OPENMPV)
    while (__nosvc_dispatch_next_8(&loc, gtid, &last, &lb, &ub, &st,
                                   &omp_task_type)) {
#else
...
#if defined(_OPENMPV)
    rc = __nosvc_dispatch_next_8(&loc, gtid, &last, &lb, &ub, &st,
                                 &omp_task_type);
#else
...
#if defined(_OPENMPV)
    rc = __nosvc_dispatch_next_4(&loc, gtid, &last, &lb, &ub, &st,
                                &omp_task_type);
#else
