...
_OMP_EXTERN void ompt_libomp_connect(ompt_start_tool_result_t *result) {
  fprintf(stderr, "ompt_libomp_connect is not a valid function call. Libompv uses ompt_libompv_connect\n");
  exit(1);
}

_OMP_EXTERN void ompt_libompv_connect(ompt_start_tool_result_t *result) {
  OMPT_VERBOSE_INIT_PRINT("libompv --> OMPT: Enter ompt_libompv_connect\n");

  // Ensure libompv callbacks have been added if not already
  __ompt_force_initialization();

  if (ompt_enabled.enabled && result) {
    OMPT_VERBOSE_INIT_PRINT("libompv --> OMPT: Connecting with libompvtarget\n");
    // Pass in the libompv lookup function so that the already registered
    ...
    // Track the object provided by libompvtarget so that the finalizer can be
    // called during OMPT finalization
    libomptarget_ompt_result = result;
  }
  OMPT_VERBOSE_INIT_PRINT("libompv --> OMPT: Exit ompt_libompv_connect\n");
}
...
_OMP_EXTERN void ompt_libompv_connect(ompt_start_tool_result_t *result) {
  fprintf(stderr, "ompt_libompv_connect is not a valid function call. Libomp uses ompt_libomp_connect\n");
  exit(1);
}
