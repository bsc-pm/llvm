...
static void replaceFrameSizeAndAlignment(coro::Shape &Shape) {
  ...
  for (CoroSizeInst *CS : Shape.CoroSizes) {
    Instruction *CSS = nullptr;
    for (User *U : CS->users()) {
      if (auto *II = dyn_cast<IntrinsicInst>(U)) {
        if (II->getIntrinsicID() == Intrinsic::coro_size_storage) {
          // Constant *C = cast<ConstantInt>(II->getOperand(0));
          // [OmpSs-2] Replace sentinel generated by the frontend
          // and replace its uses by the coro.size
          GlobalVariable *GV = cast<GlobalVariable>(II->getOperand(1));
          GlobalVariable *GVC = new GlobalVariable(
            *M, SizeConstant->getType(), /*isConstant=*/true, GlobalVariable::InternalLinkage,
            SizeConstant);
          GV->replaceAllUsesWith(GVC);
          GV->eraseFromParent();
          CSS = II;
        }
      }
