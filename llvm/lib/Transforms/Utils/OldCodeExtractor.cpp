//===- OldCodeExtractor.cpp - Pull code region into a new function -----------===//
//
...
static cl::opt<bool>
AggregateArgsOpt("old-aggregate-extracted-args", cl::Hidden,
                 cl::desc("Aggregate arguments to old-code-extracted functions"));
...
OldCodeExtractor::OldCodeExtractor(ArrayRef<BasicBlock *> BBs, DominatorTree *DT,
...
OldCodeExtractor::OldCodeExtractor(ArrayRef<BasicBlock *> BBs,
...
OldCodeExtractorAnalysisCache::OldCodeExtractorAnalysisCache(Function &F) {
  for (BasicBlock &BB : F) {
...
void OldCodeExtractorAnalysisCache::findSideEffectInfoForBlock(BasicBlock &BB) {
  for (Instruction &II : BB.instructionsWithoutDebug()) {
...
bool OldCodeExtractorAnalysisCache::doesBlockContainClobberOfAddr(
...
bool OldCodeExtractor::isLegalToShrinkwrapLifetimeMarkers(
    const OldCodeExtractorAnalysisCache &CEAC, Instruction *Addr) const {
...
BasicBlock *
OldCodeExtractor::findOrCreateBlockForHoisting(BasicBlock *CommonExitBlock) {
  ...
  BasicBlock *NewExitBlock = CommonExitBlock->splitBasicBlock(
      CommonExitBlock->getFirstNonPHIIt());
...
OldCodeExtractor::LifetimeMarkerInfo
OldCodeExtractor::getLifetimeMarkers(const OldCodeExtractorAnalysisCache &CEAC,
                                  Instruction *Addr,
...
void OldCodeExtractor::findAllocas(const OldCodeExtractorAnalysisCache &CEAC,
  ...
  // Look up allocas in the original function in OldCodeExtractorAnalysisCache, as
...
bool OldCodeExtractor::isEligible() const {
  if (Blocks.empty())
    return false;
...
void OldCodeExtractor::findInputsOutputs(ValueSet &Inputs, ValueSet &Outputs,
...
void OldCodeExtractor::severSplitPHINodesOfEntry(BasicBlock *&Header) {
...
/// outlined region.
void OldCodeExtractor::severSplitPHINodesOfExits(
...
void OldCodeExtractor::splitReturnBlocks() {
...
/// f(in0, ..., inN, out0, ..., outN)
Function *OldCodeExtractor::constructFunction(const ValueSet &inputs,
...
static void insertLifetimeMarkersSurroundingCall(
  ...
  auto insertMarkers = [&](Intrinsic::ID MarkerFunc, ArrayRef<Value *> Objects,
    ...
    for (Value *Mem : Objects) {
      ...
      Function *Func = Intrinsic::getOrInsertDeclaration(M, MarkerFunc, Mem->getType());
...
/// necessary.
CallInst *OldCodeExtractor::emitCallAndSwitchStatement(Function *newFunction,
                                                    BasicBlock *codeReplacer,
                                                    ValueSet &inputs,
                                                    ValueSet &outputs) {
...
void OldCodeExtractor::moveCodeToFunction(Function *newFunction) {
...
void OldCodeExtractor::calculateNewCallTerminatorWeights(
    BasicBlock *CodeReplacer,
  ...
  auto UpdateDbgRecordsOnInst = [&](Instruction &I) -> void {
    for (DbgRecord &DR : I.getDbgRecordRange()) {
      ...
      if (any_of(DVR.location_ops(), IsInvalidLocation)) {
        ...
        for (Value *Location : DVR.location_ops()) {
          int Res = valueInUnpackParams(Location);
          if (Res != -1)
            DVR.replaceVariableLocationOp(Location, NewFunc.getArg(Res));
        }
      }
...
Function *
OldCodeExtractor::extractCodeRegion(const OldCodeExtractorAnalysisCache &CEAC) {
...
bool OldCodeExtractor::verifyAssumptionCache(const Function &OldFunc,
...
void OldCodeExtractor::excludeArgFromAggregate(Value *Arg) {
