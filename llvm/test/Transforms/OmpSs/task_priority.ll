...
entry:
  ...
  %3 = call token @llvm.directive.region.entry() [ "DIR.OSS"([5 x i8] c"TASK\00"), "QUAL.OSS.PRIORITY"(i32 ()* @compute_priority) ], !dbg !11
  call void @llvm.directive.region.exit(token %3), !dbg !12
  %4 = call token @llvm.directive.region.entry() [ "DIR.OSS"([5 x i8] c"TASK\00"), "QUAL.OSS.FIRSTPRIVATE"(i32* %n.addr), "QUAL.OSS.PRIORITY"(i32 (i32*)* @compute_priority.1, i32* %n.addr) ], !dbg !13
  call void @llvm.directive.region.exit(token %4), !dbg !14
  %5 = call token @llvm.directive.region.entry() [ "DIR.OSS"([5 x i8] c"TASK\00"), "QUAL.OSS.FIRSTPRIVATE"(i32* %vla), "QUAL.OSS.VLA.DIMS"(i32* %vla, i64 %1), "QUAL.OSS.PRIORITY"(i32 (i32*, i64)* @compute_priority.2, i32* %vla, i64 %1), "QUAL.OSS.CAPTURED"(i64 %1) ], !dbg !15
...
define internal i32 @compute_priority() !dbg !18 {
entry:
...
define internal i32 @compute_priority.1(i32* %n) !dbg !23 {
entry:
...
define internal i32 @compute_priority.2(i32* %vla, i64 %0) !dbg !26 {
entry:
...
; CHECK: define internal void @nanos6_unpacked_priority__Z3bari0(i64* %priority) {
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = call i32 @compute_priority()
; CHECK-NEXT:   %1 = sext i32 %0 to i64
; CHECK-NEXT:   store i64 %1, i64* %priority, align 8
; CHECK-NEXT:   ret void
; CHECK-NEXT: }

; CHECK: define internal void @nanos6_ol_priority__Z3bari0(%nanos6_task_args__Z3bari0* %task_args, i64* %priority) {
; CHECK-NEXT: entry:
; CHECK-NEXT:   call void @nanos6_unpacked_priority__Z3bari0(i64* %priority)
; CHECK-NEXT:   ret void
; CHECK-NEXT: }

; CHECK: define internal void @nanos6_unpacked_priority__Z3bari1(i32* %n.addr, i64* %priority) {
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = call i32 @compute_priority.1(i32* %n.addr)
; CHECK-NEXT:   %1 = sext i32 %0 to i64
; CHECK-NEXT:   store i64 %1, i64* %priority, align 8
; CHECK-NEXT:   ret void
; CHECK-NEXT: }

; CHECK: define internal void @nanos6_ol_priority__Z3bari1(%nanos6_task_args__Z3bari1* %task_args, i64* %priority) {
; CHECK-NEXT: entry:
; CHECK-NEXT:   %gep_n.addr = getelementptr %nanos6_task_args__Z3bari1, %nanos6_task_args__Z3bari1* %task_args, i32 0, i32 0
; CHECK-NEXT:   call void @nanos6_unpacked_priority__Z3bari1(i32* %gep_n.addr, i64* %priority)
; CHECK-NEXT:   ret void
; CHECK-NEXT: }

; CHECK: define internal void @nanos6_unpacked_priority__Z3bari2(i32* %vla, i64 %0, i64* %priority) {
; CHECK-NEXT: entry:
; CHECK-NEXT:   %1 = call i32 @compute_priority.2(i32* %vla, i64 %0)
; CHECK-NEXT:   %2 = sext i32 %1 to i64
; CHECK-NEXT:   store i64 %2, i64* %priority, align 8
; CHECK-NEXT:   ret void
; CHECK-NEXT: }

; CHECK: define internal void @nanos6_ol_priority__Z3bari2(%nanos6_task_args__Z3bari2* %task_args, i64* %priority) {
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   call void @nanos6_unpacked_priority__Z3bari2(i32* %load_gep_vla, i64 %load_capt_gep, i64* %priority)
; CHECK-NEXT:   ret void
; CHECK-NEXT: }
...
!7 = !DIFile(filename: "task_priority.ll", directory: "")
...
!18 = distinct !DISubprogram(linkageName: "compute_priority", scope: !1, file: !1, type: !8, flags: DIFlagArtificial, spFlags: DISPFlagLocalToUnit | DISPFlagDefinition, unit: !0, retainedNodes: !2)
...
!23 = distinct !DISubprogram(linkageName: "compute_priority.1", scope: !1, file: !1, type: !8, flags: DIFlagArtificial, spFlags: DISPFlagLocalToUnit | DISPFlagDefinition, unit: !0, retainedNodes: !2)
!24 = !DILocation(line: 6, column: 31, scope: !25)
!25 = !DILexicalBlockFile(scope: !23, file: !7, discriminator: 0)
!26 = distinct !DISubprogram(linkageName: "compute_priority.2", scope: !1, file: !1, type: !8, flags: DIFlagArtificial, spFlags: DISPFlagLocalToUnit | DISPFlagDefinition, unit: !0, retainedNodes: !2)
