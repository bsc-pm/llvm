...
#define GENBIN(GenBinEvOp, GenBinTyCat, GenBinFirOp)                           \
  ...
  CopyOutPairs genOmpSsCall(Fortran::lower::SymMap &localSymbols,
    ...
    for (const Fortran::lower::CallInterface<
      ...
      if (!actual) {
        // Optional dummy argument for which there is no actual argument.
        mlir::Value absent = builder.genAbsentOp(loc, argTy);
      ...
      if (arg.passBy == PassBy::BaseAddress || arg.passBy == PassBy::BoxChar ||
        ...
        if (arg.mustBeMadeContiguous() &&
          ...
          // Need the box types to be exactly similar for the selectOp.
          mlir::Value convertedBox = builder.createConvert(loc, argTy, box);
          mlir::Value select = mlir::arith::SelectOp::create(builder, loc, isAllocated,
                                                             convertedBox, absent);
          localSymbols.addSymbol(*sym, select);
          caller.placeInput(arg, select);
        } else {
          ...
          if (dynamicType->category() !=
            ...
            if (mlir::isa<fir::BoxType>(box.getType()) &&
              ...
              if (arg.isOptional()) {
                ...
                box = fir::ReboxOp::create(builder, loc, actualTy,
                                           box, mlir::Value{},
                                           /*slice=*/mlir::Value{});
              }
            } else if (Fortran::lower::isParentComponent(*expr)) {
