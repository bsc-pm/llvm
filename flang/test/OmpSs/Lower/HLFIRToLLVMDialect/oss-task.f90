...
! RUN: flang-new -fc1 -fompss-2 -emit-llvm -fdisable-ompss-2-pass -mmlir --mlir-print-ir-after-all %s -o /dev/null |& tail -n +$(flang-new -fc1 -fompss-2 -emit-llvm -fdisable-ompss-2-pass -mmlir --mlir-print-ir-after-all %s -o /dev/null |& grep -n FIRToLLVMLowering | cut -f1 -d:) | FileCheck %s --check-prefix=HLFIRToLLVMDialect
PROGRAM P
  IMPLICIT NONE
  INTEGER :: X
  !$OSS TASK
