...
bool Sema::DiagnoseUseOfDecl(NamedDecl *D, ArrayRef<SourceLocation> Locs,
  ...
  // [OmpSs 4.0], 2.15 declare reduction Directive, Restrictions
  // Only the variables omp_in and omp_out are allowed in the combiner.
  // Only the variables omp_priv and omp_orig are allowed in the
  // initializer-clause.
...
ExprResult Sema::ActOnArraySubscriptExpr(Scope *S, Expr *base,
  ...
  if (base && !base->getType().isNull() &&
    ...
    return ActOnOSSArraySectionExpr(base, lbLoc, ArgExprs.front(), SourceLocation(),
...
ExprResult Sema::ActOnOSSArrayShapingExpr(Expr *Base, ArrayRef<Expr *> Shapes,
  ...
  for (int i = Shapes.size() - 1 ; i >= 0; --i) {
    ...
    Type = BuildArrayType(Type, ArraySizeModifier::Normal, NewShape, /*Quals=*/0,
...
ExprResult Sema::CheckPlaceholderExpr(Expr *E) {
  ...
  case BuiltinType::OMPArraySection:
    ...
    return ExprError();
  case BuiltinType::OSSArraySection:
    Diag(E->getBeginLoc(), diag::err_oss_array_section_use);
    return ExprError();
