...
OMPClause *SemaOpenMP::ActOnOpenMPLabelClause(Expr *ME, SourceLocation StartLoc,
                                          SourceLocation LParenLoc,
                                          SourceLocation EndLoc) {
  assert(ME && "NULL expr in Label clause");
  SmallVector<Expr *, 2> ClauseVars;
  ExprResult LabelRes = ME;

  const QualType &ConstCharPtrTy =
      SemaRef.Context.getPointerType(SemaRef.Context.CharTy.withConst());
  if (!ME->isValueDependent() && !ME->isTypeDependent() &&
      !ME->isInstantiationDependent() &&
      !ME->containsUnexpandedParameterPack()) {

    VarDecl *LabelVD =
        buildVarDecl(SemaRef, ME->getExprLoc(), ConstCharPtrTy, ".tmp.label");
    SemaRef.AddInitializerToDecl(LabelVD, ME,
                         /*DirectInit=*/false);
    if (!LabelVD->hasInit())
      return nullptr;

    if (auto *DRE = dyn_cast<DeclRefExpr>(ME)) {
    ...
    LabelRes = LabelVD->getInit();
  }

  ClauseVars.push_back(LabelRes.get());

  return OMPLabelClause::Create(SemaRef.Context, StartLoc, LParenLoc, EndLoc, ClauseVars);
}
