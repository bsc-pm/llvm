...
// OmpSs
ExprDependence clang::computeDependence(OSSArraySectionExpr *E) {
  auto D = E->getBase()->getDependence();
  if (auto *LB = E->getLowerBound())
    D |= LB->getDependence();
  if (auto *LenUp = E->getLengthUpper())
    D |= LenUp->getDependence();
  return D;
}

ExprDependence clang::computeDependence(OSSArrayShapingExpr *E) {
  auto D = E->getBase()->getDependence();
  for (auto *S : E->getShapes())
    D |= S->getDependence();
  return D;
}
