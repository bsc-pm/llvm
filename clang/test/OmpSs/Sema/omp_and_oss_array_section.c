...
void foo() {
  ...
  #pragma omp task depend(in: array[0:1], array[0;1]) // omp-error {{expected ']'}} omp-error {{expected ',' or ')' in 'depend' clause}} omp-error {{expected expression}} ossmp_error {{expected-expression}} omp-note {{to match this '['}}
  {}
