...
// CHECK-NEXT:    [[TMP3:%.*]] = call token @llvm.directive.region.entry() [ "DIR.OSS"([5 x i8] c"TASK\00"), "QUAL.OSS.SHARED"(ptr [[X]], i32 undef), "QUAL.OSS.DEP.IN"(ptr [[X]], [2 x i8] c"x\00", ptr @compute_dep.2, ptr [[X]]) ], !dbg [[DBG13:![0-9]+]]
// CHECK-NEXT:    [[TMP4:%.*]] = call i1 @llvm.directive.marker() [ "DIR.OSS"([8 x i8] c"RELEASE\00"), "QUAL.OSS.DEP.IN"(ptr [[X]], [2 x i8] c"x\00", ptr @compute_dep.3, ptr [[X]]) ], !dbg [[DBG14:![0-9]+]]
// CHECK-NEXT:    call void @llvm.directive.region.exit(token [[TMP3]]), !dbg [[DBG15:![0-9]+]]
