...
// CHECK: %0 = call token @llvm.directive.region.entry() [ "DIR.OSS"([5 x i8] c"TASK\00"), "QUAL.OSS.SHARED"(i32* %x.addr), "QUAL.OSS.DEP.REDUCTION"(i32 6000, i32* %x.addr, %struct._depend_unpack_t (i32*)* @compute_dep, i32* %x.addr), "QUAL.OSS.DEP.REDUCTION.INIT"(i32* %x.addr, void (i32*, i32*, i64)* @red_init), "QUAL.OSS.DEP.REDUCTION.COMBINE"(i32* %x.addr, void (i32*, i32*, i64)* @red_comb) ]
...
// CHECK: define internal %struct._depend_unpack_t @compute_dep(i32* %x.addr) {
// CHECK-NEXT: entry:
...
// CHECK: define internal %struct._depend_unpack_t.0 @compute_dep.1(i32* %x.addr) {
// CHECK-NEXT: entry:
