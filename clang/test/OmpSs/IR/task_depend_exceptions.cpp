...
// CHECK-NEXT:   invoke void @_ZN1SC1Ei(%struct.S* noundef %ref.tmp, i32 noundef{{( signext)?}} 0)
// CHECK-NEXT:           to label %invoke.cont unwind label %terminate.lpad
// CHECK: invoke.cont:                                      ; preds = %entry
// CHECK-NEXT:   %call = invoke noundef{{( signext)?}} i32 @_ZN1ScviEv(%struct.S* noundef %ref.tmp)
...
// CHECK-NEXT:   call void @_ZN1SD1Ev(%struct.S* noundef %ref.tmp)
