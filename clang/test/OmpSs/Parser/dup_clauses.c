...
void task_label();
#pragma oss task grid(1,1,1) grid(1,1,1) // expected-error {{directive '#pragma oss task' cannot contain more than one 'grid' clause}}
void task_label();
...
void bar() {
  ...
  #pragma oss task label("L1") label("L2") // expected-error {{directive '#pragma oss task' cannot contain more than one 'label' clause}}
  {}
  #pragma oss task for chunksize(1) chunksize(1) // expected-error {{directive '#pragma oss task for' cannot contain more than one 'chunksize' clause}}
  for (int i = 0; i < 10; ++i) {}
  #pragma oss taskloop grainsize(1) grainsize(1) // expected-error {{directive '#pragma oss taskloop' cannot contain more than one 'grainsize' clause}}
  for (int i = 0; i < 10; ++i) {}
  #pragma oss taskloop for chunksize(1) chunksize(1) // expected-error {{directive '#pragma oss taskloop for' cannot contain more than one 'chunksize' clause}}
  for (int i = 0; i < 10; ++i) {}
  #pragma oss taskloop for grainsize(1) grainsize(1) // expected-error {{directive '#pragma oss taskloop for' cannot contain more than one 'grainsize' clause}}
  for (int i = 0; i < 10; ++i) {}
